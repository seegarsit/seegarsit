"""
Simple IT Ticketing Website — single-file Flask app

Features
- Create, list, search, and filter tickets (by status, priority, branch)
- View a ticket, add comments, change status, and assign to someone
- SQLite database (app.db) using Python's built‑in sqlite3
- Bootstrap UI via CDN for a clean look
...
Note: This is intentionally simple (no authentication). Add auth (Flask-Login, SSO, etc.) if you deploy broadly.
"""
...
# ---------------------------
# Templates (Jinja) — kept inline for single‑file simplicity
# ---------------------------
...
<nav class="navbar navbar-expand-lg navbar-dark mb-4">
  <div class="container">
    <a class="navbar-brand" href="{{ url_for('list_tickets') }}">🛠️ IT Tickets</a>
    <div class="ms-auto">
  ...
  <form method="post">
    <div class="row g-3">
      ...
      <div class="col-md-4">
        ...
        <input name="requester_name" class="form-control">
      </div>
      <div class="col-md-4">
...
# Jinja needs a loader when using render_template_string extends
...
# Map endpoints to our string templates
@app.route("/index_template")
def _index_template_preview():  # debug helper
    return render_template_string(INDEX_HTML, tickets=[], statuses=STATUSES, priorities=PRIORITIES)
